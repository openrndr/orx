...
class TestEdgesForVertex {
    ...
    fun testEdgesEndingAtVertex() {
        ...
        // Vertex 1: edges ending at 1 should be (0 -> 1) and (2 -> 1)
        val edges1 = dcel.edgesEndingAtVertex(1)
        assertEquals(2, edges1.size)
        // edgesEndingAtVertex returns edges where the next vertex is 1
        assertTrue(edges1.all { dcel.halfEdges[dcel.halfEdges[it].nextEdge].vertex == 1 })

        val edges3 = dcel.edgesEndingAtVertex(3)
        assertEquals(2, edges3.size)
        assertTrue(edges3.all { dcel.halfEdges[dcel.halfEdges[it].nextEdge].vertex == 3 })

        val edges0 = dcel.edgesEndingAtVertex(0)
        assertEquals(1, edges0.size)
        assertEquals(0, dcel.halfEdges[dcel.halfEdges[edges0[0]].nextEdge].vertex)
    }

    @Test
    fun testIsolatedVertex() {
        val dcel = Dcel()
        dcel.vertices.add(org.openrndr.extra.mesh.dcel.Vertex(Vector3.ZERO, -1))
        val edges = dcel.edgesForVertex(0)
